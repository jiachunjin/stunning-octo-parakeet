...
class Transformer_Projector(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.in_proj = nn.Linear(config.high_dim, config.hidden_size)
        self.pos_embed = nn.Parameter(torch.zeros(1, config.num_tokens, config.hidden_size), requires_grad=False)
        self.blocks = nn.ModuleList([
            Block(config.hidden_size, config.num_heads, mlp_ratio=4) for _ in range(config.depth)
        ])
        self.out_proj = nn.Linear(config.hidden_size, config.down_dim)
        self.init_weights()
