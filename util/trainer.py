...
class Trainer:
    def __init__(self, config):
        ...
        if self.accelerator.is_main_process:
            self.accelerator.init_trackers(config.train.wandb_proj, config=flatten_dict(config))
    ...
    def _load_optimizer(self):
        params_to_learn = list(p for p in self.model.parameters() if p.requires_grad)
        ...
        self.accelerator.print(f"Learnable parameters: {sum(p.numel() for p in params_to_learn if p.requires_grad) / 1e6} M")
